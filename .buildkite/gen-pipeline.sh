...
run_test() {
  ...
  if [[ ${queue} == "gpu" ]]; then
    command="export CUDA_VISIBLE_DEVICES=\$(if [[ \${BUILDKITE_AGENT_NAME} == *\"-1\" ]]; then echo \"0,1,2,3\"; else echo \"4,5,6,7\"; fi) && (${command})"
  fi
...
for test in ${tests[@]}; do
  ...
  run_test "${test}" "${queue}" \
    ...
    "bash -c \"cd /horovod/test && (echo test_*.py | xargs -n 1 ${MPIRUN_COMMAND} pytest -v --capture=no)\""

  run_test "${test}" "${queue}" \
    ":muscle: Test TensorFlow MNIST (${test})" \
    "bash -c \"${MPIRUN_COMMAND} python /horovod/examples/tensorflow_mnist.py\""

  if [[ ${test} != *"tf1_1_0"* && ${test} != *"tf1_6_0"* ]]; then
    run_test "${test}" "${queue}" \
      ":muscle: Test TensorFlow Eager MNIST (${test})" \
      "bash -c \"${MPIRUN_COMMAND} python /horovod/examples/tensorflow_mnist_eager.py\""
  fi

  run_test "${test}" "${queue}" \
    ":muscle: Test Keras MNIST (${test})" \
    "bash -c \"${MPIRUN_COMMAND} python /horovod/examples/keras_mnist_advanced.py\""

  run_test "${test}" "${queue}" \
    ":muscle: Test PyTorch MNIST (${test})" \
    "bash -c \"${MPIRUN_COMMAND} python /horovod/examples/pytorch_mnist.py\""

  run_test "${test}" "${queue}" \
    ":muscle: Test MXNet MNIST (${test})" \
    "bash -c \"OMP_NUM_THREADS=1 ${MPIRUN_COMMAND} python /horovod/examples/mxnet_mnist.py\""

  run_test "${test}" "${queue}" \
    ":muscle: Test Stall (${test})" \
    "bash -c \"${MPIRUN_COMMAND} python /horovod/test/test_stall.py\""
